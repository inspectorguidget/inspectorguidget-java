...
public class Command {
	...
	private void optimiseStatementEntries() {
		...
		while(i < statements.size()) {
			int j = 0;
			final CommandStatmtEntry stat1 = statements.get(i);
			while(j < statements.size()) {
				final CommandStatmtEntry stat2 = statements.get(j);
	...
	public @NotNull List<CodeBlockPos> getOptimalCodeBlocks() {
		return Stream.concat(
				statements.stream()
					.filter(stat -> !stat.getStatmts().isEmpty())
					.map(stat -> new CodeBlockPos(stat.getStatmts().get(0).getPosition().getCompilationUnit().getFile().toString(),
												stat.getLineStart(), stat.getLineEnd())),
				conditions.stream()
					.map(stat -> new CodeBlockPos(stat.realStatmt.getPosition().getCompilationUnit().getFile().toString(),
												stat.realStatmt.getPosition().getLine(), stat.realStatmt.getPosition().getEndLine())))
			.collect(Collectors.groupingBy(triple -> triple.file))
			.values()
			.parallelStream()
			.map(triples -> triples.stream().sorted(Comparator.comparingInt(o -> o.startLine)).collect(Collectors.toList()))
			.map(triples -> {
				int i = 0;
				CodeBlockPos ti;
				CodeBlockPos tj;
				...
				return triples.stream().sorted(Comparator.comparingInt(o -> o.startLine));
...
//						filter(stat -> stat.getPosition() == null).findFirst().ifPresent(statnull -> System.out.println("NO POSITION: " + statnull));
		return Stream
			.concat(statements.stream()
					.filter(stat -> !stat.getStatmts().isEmpty())
					.map(stat -> stat.getStatmts().get(0).getPosition()),
				conditions.stream()
					.map(stat -> stat.realStatmt.getPosition()))
			.mapToInt(pos-> pos.getEndLine()-pos.getLine()+1)
			.sum();
	}
